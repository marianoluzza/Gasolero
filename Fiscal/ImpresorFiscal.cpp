...
{
	unsigned Velocidades[] = { 9600, 19200, 38400, 57600, 4800, 2400, 1200};
...
ImpresorFiscal::EnviarComandoFiscal(const std::string &Comando) throw (Excepcion)
{
	...
	// Guardamos la copia en UltimoComando
...
ImpresorFiscal::EvaluarRespuesta(unsigned short &ErrorFiscal)
{
	...
	bool Cajon = !(Status & Mascara(ESTADO_IMPRESOR_CAJON_ABIERTO)) != 0;
	if (Cajon != DrawerOpened)
	{
		DrawerOpened = Cajon;
		RaiseEvent(EVENTO_DE_ESTADO_CAJON, Cajon);
	}
...
//
bool
ImpresorFiscal::IndicadorDeDecumentoPrevioCancelado () throw(Excepcion)
{
	// Enviamos el comando fiscal
	SolicitarEstado ();
...
ImpresorFiscal::DescripcionEstadoImpresor (EstadosImpresor Estado) throw(Excepcion)
{
	...
	if (DescripcionBitEstadoImpresor.size() == 0)
	{
		...
		DescripcionBitEstadoImpresor[ESTADO_IMPRESOR_CAJON_ABIERTO]			= "El cajón de dinero está cerrado";
	}
							...
							TiposDeCorteDePapel TipoDeCorte) throw (Excepcion)
{
	...
	EnviarComandoFiscal(OpCode(CMD_CONFIGURE_CONTROLLER_BY_BLOCK) + FS +
						...
						Caracter (NumeroDeCopiasMaximo) + FS +
...
//
double
ImpresorFiscal::ImprimirPago (const std::string &Texto, double Monto) throw (Excepcion)
{
