...
ImpresorFiscal16Bits::UltimoDocumentoFueCancelado () throw(Excepcion)
{
	...
	return IndicadorDeDecumentoPrevioCancelado ();
}
							...
							const TiposDeModoImpresion *ModoImpresion) throw (Excepcion)
{
	...
	EnviarComandoFiscal(OpCode(CMD_SET_CONFIG_FULL) + FS +
						...
						(NumeroDeCopiasMaximo		 ? Caracter (*NumeroDeCopiasMaximo)			: "") + FS +
...
ImpresorFiscal16Bits::ObtenerConfiguracion (RTA_ObtenerConfiguracion *R) throw (Excepcion)
{
	...
	if (R != NULL)
	{
		...
		R->NumeroDeCopiasMaximo			= static_cast<ImpresorFiscal::NumerosDeCopias>(RespuestaString (5, "Obtener Configuracion del Controlador") [0]);
...
ImpresorFiscal16Bits::AbrirDNFH (DocumentosNoFiscalesHomologados Tipo, const std::string &Nro) throw (Excepcion)
{
	...
	// Seteamos el tipo de DNFH apropiado

	// La siguiente tabla contiene los Tipos de DNFH que
	// deben ser convertidos antes de armar el comando.
	struct IdentificadorDNFH
	{
		DocumentosNoFiscalesHomologados Tipo;
		DocumentosNoFiscalesHomologados Id;
	};

	IdentificadorDNFH Identificadores[] =
	{
		{
			TICKET_NOTA_CREDITO_A,	NOTA_CREDITO_A
		},
		{
			TICKET_NOTA_CREDITO_B,	NOTA_CREDITO_B
		},
		{
			TICKET_RECIBO_X,		RECIBO_X
		},
		{
			TICKET_PAGARE,			PAGARE
		}
	};

	const unsigned MAX_TIPOS = sizeof(Identificadores) / sizeof(IdentificadorDNFH);
...
//
void
ImpresorFiscal16Bits::ImprimirInfoEnDNFH (const unsigned IDCampo, const std::string &Texto) throw (Excepcion)
{
